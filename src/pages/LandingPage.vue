<template>
  <q-page class="bg-grey-10 text-white" style="min-height: 100vh">
    ...
    <section class="hero q-pa-xl text-center">
      ...
      <div class="text-subtitle1 text-grey-4 q-mb-md" style="max-width: 640px; margin: 0 auto">
      ...
      <div class="row justify-center q-gutter-md q-mb-xl">
        ...
        <q-chip icon="precision_manufacturing" color="dark" text-color="white" outline>
          Tools &amp; Machines
        </q-chip>
      ...
      <div class="row q-gutter-lg justify-center">
        ...
        <q-card dark class="feature-card col-12 col-md-auto">
          <q-card-section class="card-header">
            <q-icon name="location_on" size="28px" color="positive" class="q-mr-sm" />
            ...
            <q-btn-dropdown flat dense icon="more_vert" color="grey-5">
              <q-list dark>
                ...
                <q-item clickable v-close-popup @click="locationAction('delete')" :disable="!locationDirty">
                  <q-item-section avatar><q-icon name="delete" color="negative" /></q-item-section>
                  <q-item-section class="text-negative">Delete draft</q-item-section>
                </q-item>
              </q-list>
            </q-btn-dropdown>
          </q-card-section>
          <q-card-section class="q-pt-none">
            ...
            <LocationEditor v-model="locationDraft" label="coordinates" />
          </q-card-section>
          <q-card-actions align="right">
            <q-btn flat label="Reset" icon="undo" :disable="!locationDirty" @click="locationAction('reset')" />
            <q-btn
              color="positive"
          ...
          <q-banner v-if="locationMsg" dense :class="locationMsgClass" class="q-mx-md q-mb-sm">
            {{ locationMsg }}
          </q-banner>
        </q-card>
        ...
        <q-card dark class="feature-card col-12 col-md-auto">
          <q-card-section class="card-header">
            <q-icon name="precision_manufacturing" size="28px" color="secondary" class="q-mr-sm" />
            ...
            <q-btn-dropdown flat dense icon="more_vert" color="grey-5">
              <q-list dark>
                ...
                <q-item clickable v-close-popup @click="machineAction('delete')" :disable="!machineDirty">
                  <q-item-section avatar><q-icon name="delete" color="negative" /></q-item-section>
                  <q-item-section class="text-negative">Delete draft</q-item-section>
                </q-item>
              </q-list>
          ...
          <q-card-actions align="right">
            <q-btn flat label="Reset" icon="undo" :disable="!machineDirty" @click="machineAction('reset')" />
            <q-btn
              color="secondary"
          ...
          <q-banner v-if="machineMsg" dense :class="machineMsgClass" class="q-mx-md q-mb-sm">
            {{ machineMsg }}
          </q-banner>
        </q-card>
        ...
        <q-card dark class="feature-card col-12 col-md-auto">
          <q-card-section class="card-header">
            <q-icon name="menu_book" size="28px" color="primary" class="q-mr-sm" />
            <span class="text-h6">Know-How</span>
            <q-space />
            <q-btn
              flat dense icon="schema" color="grey-5"
            ...
            <q-btn
              flat dense icon="verified" color="grey-5"
            ...
            <q-btn-dropdown flat dense icon="more_vert" color="grey-5">
              <q-list dark>
                ...
                <q-item clickable v-close-popup @click="knowHowAction('delete')" :disable="!knowHowDirty">
                  <q-item-section avatar><q-icon name="delete" color="negative" /></q-item-section>
                  <q-item-section class="text-negative">Delete draft</q-item-section>
                </q-item>
              </q-list>
          ...
          <q-card-actions align="right">
            <q-btn flat label="Reset" icon="undo" :disable="!knowHowDirty" @click="knowHowAction('reset')" />
            <q-btn
              color="primary"
          ...
          <q-banner v-if="knowHowMsg" dense :class="knowHowMsgClass" class="q-mx-md q-mb-sm">
            {{ knowHowMsg }}
          </q-banner>
        </q-card>
      ...
      <q-card dark style="min-width: 420px; max-width: 92vw">
        ...
        <q-card-section class="q-pt-none">
          ...
          <q-list v-else dark separator>
            ...
            >
              <q-item-section>
                <q-item-label>{{ entry.name || entry.key || '(unnamed)' }}</q-item-label>
...
import { KnowHow, Location, MachineInstance } from '@trace.market/types';
import { useAccountStore } from 'src/stores/account';
import LocationEditor from 'src/components/editors/LocationEditor.vue';
...
// ── Draft state ──────────────────────────────────────────────────
const locationDraft = ref<Location>(clone(defaultLocation));
...
const locationMsgClass = computed(() =>
  locationMsg.value.startsWith('✓') ? 'bg-positive text-white' : 'bg-negative text-white'
);
const machineMsgClass = computed(() =>
  machineMsg.value.startsWith('✓') ? 'bg-positive text-white' : 'bg-negative text-white'
);
const knowHowMsgClass = computed(() =>
  knowHowMsg.value.startsWith('✓') ? 'bg-positive text-white' : 'bg-negative text-white'
);
...
watch(locationDraft, () => { locationDirty.value = true; }, { deep: true });
watch(machineDraft,  () => { machineDirty.value  = true; }, { deep: true });
watch(knowHowDraft,  () => { knowHowDirty.value  = true; }, { deep: true });
...
async function readFeed(topic: string): Promise<any[]> {
  ...
  const reader = bee.makeFeedReader('sequence', t, accountStore.account.address as string);
  try {
...
async function writeFeed(topic: string, items: any[]): Promise<void> {
  ...
  await writer.upload(swarmBatchId, upload.reference, { index: nextIndexHex as any });
}

// ── upsert helper ────────────────────────────────────────────────
async function upsertEntry(existing: any[], value: any, key: string): Promise<any[]> {
  const list = [...existing];
...
async function locationAction(action: 'save' | 'load' | 'reset' | 'delete') {
  if (action === 'reset') { locationDraft.value = clone(defaultLocation); locationDirty.value = false; return; }
  if (action === 'load') { await openLoadDialog('tm-editor-location', 'location'); return; }
  if (action === 'delete') { locationDraft.value = clone(defaultLocation); locationDirty.value = false; setMsg('location', '✓ Draft cleared'); return; }

  locationSaving.value = true;
  try {
    const existing = await readFeed('tm-editor-location');
    const key = `${locationDraft.value.coordinates[0]},${locationDraft.value.coordinates[1]}`;
...
async function machineAction(action: 'save' | 'load' | 'reset' | 'delete') {
  if (action === 'reset') { machineDraft.value = clone(defaultMachineInstance); machineDirty.value = false; return; }
  if (action === 'load') { await openLoadDialog('tm-editor-machine', 'machine'); return; }
  if (action === 'delete') { machineDraft.value = clone(defaultMachineInstance); machineDirty.value = false; setMsg('machine', '✓ Draft cleared'); return; }

  machineSaving.value = true;
  try {
    const existing = await readFeed('tm-editor-machine');
    const key = machineDraft.value.ownerId || machineDraft.value.category || String(Date.now());
...
async function knowHowAction(action: 'save' | 'load' | 'reset' | 'delete') {
  if (action === 'reset') { knowHowDraft.value = clone(defaultKnowHow); knowHowDirty.value = false; return; }
  if (action === 'load') { await openLoadDialog('tm-editor-knowHow', 'knowHow'); return; }
  if (action === 'delete') { knowHowDraft.value = clone(defaultKnowHow); knowHowDirty.value = false; setMsg('knowHow', '✓ Draft cleared'); return; }

  knowHowSaving.value = true;
  try {
    const existing = await readFeed('tm-editor-knowHow');
    const key = knowHowDraft.value.hash || knowHowDraft.value.owner || String(Date.now());
...
function applyFeedEntry(entry: any) {
  ...
  if (loadDialogTarget === 'location') { locationDraft.value = v as Location; locationDirty.value = false; }
  else if (loadDialogTarget === 'machine') { machineDraft.value = v as MachineInstance; machineDirty.value = false; }
  else { knowHowDraft.value = v as KnowHow; knowHowDirty.value = false; }
